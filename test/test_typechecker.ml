...
let%expect_test "synth: unit literal" =
  print_string (synth_ok [] Unit);
  [%expect {| unit |}]

let%expect_test "synth: int literal" =
  print_string (synth_ok [] (Lit 5));
  [%expect {| int |}]

let%expect_test "synth: Ord variable" =
  print_string (synth_ok [("x", Ord, TInt)] (Var "x"));
  [%expect {| int |}]

let%expect_test "synth: Disc variable (DVar)" =
  print_string (synth_ok [("x", Disc, TInt)] (Var "x"));
  [%expect {| int |}]

let%expect_test "synth: arithmetic add" =
  print_string (synth_ok [] (BinOp { op = Add; e1 = Lit 1; e2 = Lit 2 }));
  [%expect {| int |}]

let%expect_test "synth: equality returns sum" =
  print_string (synth_ok [] (BinOp { op = Eq; e1 = Lit 1; e2 = Lit 2 }));
  [%expect {| (unit + unit) |}]

let%expect_test "synth: pair" =
  print_string (synth_ok [] (Pair { e1 = Lit 1; e2 = Unit }));
  [%expect {| (int * unit) |}]

let%expect_test "synth: fst" =
  print_string (synth_ok [] (ProjL (Pair { e1 = Lit 1; e2 = Unit })));
  [%expect {| int |}]

let%expect_test "synth: snd" =
  print_string (synth_ok [] (ProjR (Pair { e1 = Lit 1; e2 = Unit })));
  [%expect {| unit |}]

let%expect_test "synth: inl" =
  print_string (synth_ok [] (Inl { e = Lit 1; t = TSum (TInt, TUnit) }));
  [%expect {| (int + unit) |}]

let%expect_test "synth: inr" =
  print_string (synth_ok [] (Inr { e = Unit; t = TSum (TInt, TUnit) }));
  [%expect {| (int + unit) |}]
...
let%expect_test "synth: case" =
  let e = Case { e  = Inl { e = Lit 1; t = TSum (TInt, TUnit) };
                 x  = "a"; e1 = Lit 0;
                 y  = "b"; e2 = Lit 1 } in
  print_string (synth_ok [] e);
  [%expect {| int |}]

let%expect_test "synth: lambda" =
  print_string (synth_ok [] (Lam { x = "x"; a = TInt; e = Var "x" }));
  [%expect {| (int -> int) |}]
...
let%expect_test "synth: bot" =
  print_string (synth_ok [] (Bot LUnit));
  [%expect {| unit |}]

let%expect_test "synth: bot set" =
  print_string (synth_ok [] (Bot (LPow FInt)));
  [%expect {| set int |}]

let%expect_test "synth: join" =
  print_string (synth_ok [] (Join { e1 = Bot LUnit; e2 = Bot LUnit }));
  [%expect {| unit |}]

let%expect_test "synth: singleton with Disc var" =
  print_string (synth_ok [("x", Disc, TInt)] (Sing (Var "x")));
  [%expect {| set int |}]

let%expect_test "synth: for" =
  let e = For { e2 = Bot (LPow FInt); x = "x";
                e1 = Sing (Var "x") } in
  print_string (synth_ok [("x", Disc, TInt)] e);
  [%expect {| set int |}]

let%expect_test "synth: box with Disc var survives restriction" =
  print_string (synth_ok [("x", Disc, TInt)] (Box (Var "x")));
  [%expect {| [int] |}]
...
let%expect_test "synth error: unbound variable" =
  print_string (synth_err [] (Var "x"));
  [%expect {| unbound variable: x |}]

let%expect_test "synth error: arithmetic on non-int" =
  print_string (synth_err [] (BinOp { op = Add; e1 = Unit; e2 = Lit 1 }));
  [%expect {| arithmetic operands must both have type int |}]

let%expect_test "synth error: apply non-function" =
  print_string (synth_err [] (App { e1 = Lit 1; e2 = Lit 2 }));
  [%expect {| expected a function type, got int |}]

let%expect_test "synth error: argument type mismatch" =
  let e = App { e1 = Lam { x = "x"; a = TInt; e = Var "x" }; e2 = Unit } in
  print_string (synth_err [] e);
  [%expect {| argument type mismatch: expected int, got unit |}]

let%expect_test "synth error: fst of non-product" =
  print_string (synth_err [] (ProjL (Lit 1)));
  [%expect {| expected a product type, got int |}]

let%expect_test "synth error: case scrutinee not a sum" =
  let e = Case { e = Lit 1; x = "a"; e1 = Lit 0; y = "b"; e2 = Lit 1 } in
  print_string (synth_err [] e);
  [%expect {| case scrutinee must have a sum type, got int |}]
...
let%expect_test "synth error: case branches disagree" =
  let e = Case { e  = Inl { e = Lit 1; t = TSum (TInt, TUnit) };
                 ...
                 y  = "b"; e2 = Unit } in
  print_string (synth_err [] e);
  [%expect {| case branches have incompatible types: int vs unit |}]

let%expect_test "synth error: box with Ord var fails" =
  print_string (synth_err [("x", Ord, TInt)] (Box (Var "x")));
  [%expect {| unbound variable: x |}]

let%expect_test "synth error: singleton with Ord var fails" =
  print_string (synth_err [("x", Ord, TInt)] (Sing (Var "x")));
  [%expect {| unbound variable: x |}]

let%expect_test "synth error: join type mismatch" =
  print_string (synth_err [] (Join { e1 = Bot LUnit; e2 = Lit 1 }));
  [%expect {| join: operands have different types: unit vs int |}]

let%expect_test "synth error: for over non-set" =
  let e = For { e2 = Lit 1; x = "x"; e1 = Bot LUnit } in
  print_string (synth_err [] e);
  [%expect {| for: set expression must have type set T, got int |}]

let%expect_test "synth error: let-box with non-box" =
  let e = LetBox { x = "x"; e1 = Lit 1; e2 = Var "x" } in
  print_string (synth_err [] e);
  [%expect {| let[]: expected a box type, got int |}]

let%expect_test "synth error: fix body type mismatch" =
  let e = Fix { x = "x"; l = LUnit; e = Lit 1 } in
  print_string (synth_err [] e);
  [%expect {| fix: body type int does not match declared lattice type unit |}]
